...
//!    [`StdinPlan::TerminalSlave`], fd 1 otherwise. Measured on Darwin 25.5.0 with a three-way
//!    probe: with the slave on fd 0, `setsid()` **alone** already makes the pty the child's
//!    controlling terminal and the ioctl is redundant; with stdin a pipe and the slave on fd 1,
//!    `setsid()` alone leaves `tcgetsid(master)` at `ENOTTY` and only `ioctl(1, TIOCSCTTY, 0)`
//!    claims it. So the call is load-bearing exactly where the brief's fd number would have
//!    broken.)*
...
pub struct PtyHost {
    ...
    /// Called between the `r` record and `TIOCSWINSZ`, so a test can force the interleaving that
    /// makes the ordering observable. See [`PtyHost::resize`].
    #[cfg(test)]
    resize_hook: Option<Box<dyn Fn() + Send + Sync>>,
}
...
impl PtyHost {
    ...
    /// Run `hook` between the `r` record and `TIOCSWINSZ`, so a test can **force** the
    /// interleaving the ordering claim is about instead of racing for it. See
    /// `a_resize_record_precedes_the_output_it_explains`.
    #[cfg(test)]
    ...
    /// **Record, then `TIOCSWINSZ`, then `killpg(pgid, SIGWINCH)`.**
    ///
    /// This inverts `tests/fixtures/s2/ptyhost.py`, which sizes first and records second, and the
    /// inversion is deliberate. The fixture host cannot crash between the two steps in any way that
    /// matters — it is a script whose output is read once, afterwards. The supervisor can, and the
    /// two failure modes are not symmetric: an `r` for a resize that never happened replays as a
    /// harmless reflow to a size nothing was painted at, while **output painted at a size no record
    /// mentions is unreplayable** — the emulator has no way to learn the geometry, and every
    /// absolute cursor address after it lands in the wrong place.
    ///
    ...
    /// — an optimisation paid for in evidence.
    pub fn resize(&self, size: WinSize) -> io::Result<()> {
        self.shared
            ...
            .resize(size)?;
        #[cfg(test)]
        if let Some(hook) = &self.resize_hook {
            hook();
        }
    ...
    pub fn shutdown(&mut self) -> io::Result<Option<std::process::ExitStatus>> {
        let status = match self.child.as_mut() {
            None => None,
            Some(child) => {
                crate::run::kill_process_tree(child.pid());
                Some(child.wait()?)
            }
        };
